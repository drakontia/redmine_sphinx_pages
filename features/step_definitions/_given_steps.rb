...
Given /^I am viewing the sphinx page$/ do
  visit url_for(:controller => :projects, :action => :show, :id => @project.identifier, :only_path=>true)
  assert_page_loaded(page)
  click_link("Sphinx_Pages")
  page.current_path.should == url_for(:controller => :sphinx_pages, :action => :show, :project_id => @project.identifier, :only_path=>true)
  assert_page_loaded(page)
end
...
Given /^the (.*) project has the sphinx_pages plugin enabled$/ do |project_id|
  Rails.cache.clear
  @project = get_project(project_id)
  @project.should_not be_nil

  # Enable the sphinx_pages plugin
  @project.enabled_modules << EnabledModule.new(:name => 'sphinx_pages')

  # Configure the story and task trackers
  story_trackers = Tracker.find(:all).map{|s| "#{s.id}"}
  task_tracker = "#{Tracker.create!(:name => 'Task').id}"
  plugin = Redmine::Plugin.find('redmine_sphinx_pages')
